...
export interface ModuleListPageProps {
  /** 后端 module 参数（PRE_MEETING / NOTICE / DOCUMENT / RESULT_REPORT / PRICE_COMPARE / FRAMEWORK_EXPLANATION） */
  moduleKey: string;
  /** 基础查询参数（如 type=SINGLE&preMeetingRequired=true） */
  baseParams?: Record<string, unknown>;
  /** 特有筛选项（排在公共筛选之后） */
  extraFilters?: ModuleListFilterField[];
  /** 特有表格列（插在「采购类型」之后、「发布时间」之前） */
...
export default function ModuleListPage({
  ...
  const filters = useMemo(() => [MODULE_STATUS_FILTER, PUBLISHED_AT_FILTER, ...extraFilters], [extraFilters]);
  ...
  const buildParams = useCallback(
    (values: Record<string, any>) => {
      const params: Record<string, any> = { module: moduleKey, page, pageSize, ...(baseParams ?? {}) };
    ...
    [moduleKey, page, pageSize, baseParams],
  );
  ...
  const columns: ColumnsType<ModuleListRow> = [
    ...
    },
    {
      ...
      fixed: 'right',
      render: (_v, row) => {
        const editable = rowEditable ? rowEditable(row) : moduleStatusOf(row).value === 'EDITING';
        const moreItems = onEdit
          ? [
              {
                key: 'edit',
                label: '编辑',
